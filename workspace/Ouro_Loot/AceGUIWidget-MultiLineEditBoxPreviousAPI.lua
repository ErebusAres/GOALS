--[[
Up until r920-something, MultiLineEditBox had a public method call to remove the
"Accept" button and expand the edit area to take up the remaining space.  Until
that functionality is added back into the rewritten widget, we keep this around
under a different name.  The file is the same as MLEBv11, but the "editbox"
remember has been renamed to "editBox" to keep in line with the newer code (so
that our code reaching inside doesn't need to change when this finally get updated
in the future.
-farmbuyer
]]
...
do
	local Type = "MultiLineEditBoxPreviousAPI"

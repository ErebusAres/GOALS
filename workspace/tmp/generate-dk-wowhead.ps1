...
$dkEntries = $entries | Where-Object { $_.Class -eq 'DEATHKNIGHT' }
$out = New-Object System.Collections.Generic.List[string]

foreach ($entry in $dkEntries) {
  $specName = $entry.Spec
  ...
  $id = 'DEATHKNIGHT_' + $specId + '_' + $baseTier + $idSuffix + '_WOWHEAD'

  $tierName = $baseTier.Replace('WOTLK_', '')
  if ($tierName -eq 'PRE') { $tierName = 'PRE' }
  $labelName = if ($entry.Label) { ' (' + $entry.Label + ')' } else { '' }
  $name = $tierName + ' Deathknight ' + $specName + $labelName + ' - Wowhead WotLK'
...
$out | Set-Content -Path workspace\tmp\dk-wowhead-builds.lua

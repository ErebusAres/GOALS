...
local function ParseAndApplyPlayerPoints(data)
    for entry in string.gmatch(data, "([^;]+)") do
        ...
        if playerName and points and class then
            EnsurePlayerPointsEntry(playerName, class)
...
local function SerializePlayerPoints(playerPoints)
    ...
    for playerName, info in pairs(playerPoints) do
        serialized = serialized .. playerName .. ":" .. info.points .. ":" .. info.class .. ";"
    end
    return serialized
end
...
local function SendPlayerPointsToGroup()
    ...
    local serializedData = SerializePlayerPoints(playerPoints)
    if IsInRaid() then
        C_ChatInfo.SendAddonMessage(ADDON_PREFIX, serializedData, "RAID")
    elseif IsInGroup() then
        C_ChatInfo.SendAddonMessage(ADDON_PREFIX, serializedData, "PARTY")
    end
end
...
SLASH_GOBROADCAST1 = "/gobroadcast"
SlashCmdList["GOBROADCAST"] = function()
...
local function OnEvent(self, event, ...)
    ...
    elseif event == "CHAT_MSG_ADDON" then
        ...
        if prefix == ADDON_PREFIX and sender ~= UnitName("player") then
            ParseAndApplyPlayerPoints(message)
            SendToGoalsChat("Received updated player points from " .. sender)
        end
...
eventFrame:RegisterEvent("CHAT_MSG_ADDON")  -- Register for incoming addon messages

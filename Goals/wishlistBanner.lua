...
local _, private = ...
private = private or {}
if not private.sendSync then
	function private.sendSync() end
end
...
if not DBM then
	...
	DBM.Options.PlayBBLoot = false
	DBM.Options.EnableBB = true
	DBM.Options.BBFontSize = 0
	DBM.Options.BBFontStyle = ""
	DBM.Options.BBFontShadow = true
	DBM.Options.OverrideBBFont = false
	DBM.Options.BBFont = "standardFont"
	function DBM:Debug() end
	function DBM:Schedule(delay, func, ...)
		if type(func) ~= "function" then
			return
		end
		local args = { ... }
		if delay and delay > 0 and Goals.Delay then
			Goals:Delay(delay, function()
				func(unpack(args))
			end)
		else
			func(unpack(args))
	...
	function DBM:GetModByName() return nil end
	function DBM:GetCIDFromGUID() return nil end
end

if not DBM_CORE_L then
	DBM_CORE_L = {
		BOSS_YOU_DEFEATED = "Wishlist items found",
		BOSS_KILL_SUBTITLE = "",
	}
end
...
local next, pairs, tconcat, tinsert, wipe = next, pairs, table.concat, tinsert, table.wipe or wipe
if not wipe then
	function wipe(tbl)
...
local SOUNDKIT = {
	["UI_RAID_BOSS_DEFEATED"] = "Sound\\Interface\\RaidWarning.ogg",
	["UI_PERSONAL_LOOT_BANNER"] = "Sound\\Interface\\RaidWarning.ogg",
}
...
local BossBanner = CreateFrame("Frame", "GoalsWishlistBossBanner", UIParent)
BossBanner:Hide()
...
local function SetItemButtonQuality(button, quality)
	if button and button.IconBorder then
		button.IconBorder:SetTexture("Interface\\AddOns\\Goals\\Texture\\BossBannerToast\\WhiteIconFrame")
...
local function BossBanner_OnEvent(self, event)
	if event == "LOOT_OPENED" and not DBM.Options.PlayBBLoot then
		return
	end
...
function Goals:ShowWishlistFoundAlertLocal(itemLinks, allowSound)
	...
	for i = #banner.pendingLoot, 1, -1 do
		banner.pendingLoot[i] = nil
	end
	banner.animState = nil
	banner.animTimeLeft = nil
	banner.lootShown = 0
	for i = 1, #banner.LootFrames do
		banner.LootFrames[i]:Hide()
	end

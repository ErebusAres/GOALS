...
local function CheckForWipe()
    -- Check if the player is in a party or raid
    if IsInRaid() then
        -- If in a raid, check all raid members
        for i = 1, GetNumGroupMembers() do
            local unit = "raid"..i
            if UnitExists(unit) and not UnitIsDeadOrGhost(unit) then
                return false -- If at least one raid member is alive, no wipe
            end
        end
    elseif IsInGroup() then
        -- If in a party, check all party members
        for i = 1, GetNumGroupMembers() - 1 do -- Party members are "party1" to "party4"
            local unit = "party"..i
            if UnitExists(unit) and not UnitIsDeadOrGhost(unit) then
                return false -- If at least one party member is alive, no wipe
            end
        end
        ...
        -- If the player is solo, there's no wipe scenario
        return false
    end

    return true -- If we get here, everyone is dead (or in ghost form)
end
